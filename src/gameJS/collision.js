import { playerArray, bulletArray, platformArray, boss } from '../index';
...
export function hitDetection() {
  for (let i = 0; i < bulletArray.bullets.length; i++) {
    for (let j = 0; j < playerArray.players.length; j++) {
      if (
        bulletArray.bullets[i].x - playerArray.players[j].x < 20 &&
        bulletArray.bullets[i].x - playerArray.players[j].x > 0 &&
        bulletArray.bullets[i].y - playerArray.players[j].y < 40 &&
        bulletArray.bullets[i].y - playerArray.players[j].y > 0
      ) {
...
//Collision detection for player <-> platforms
    ...
    for (let j = 0; j < platformArray.plat.length; j++) {
      if (
        playerArray.players[i].y >
          platformArray.plat[j].y - playerArray.players[i].height &&
        ...
        if (playerArray.players[i].health > 1) {
          ...
          playerArray.players[i].y =
            platformArray.plat[j].y - playerArray.players[i].height + 4;
        }
      }
    }
  }
...
//Collision detection for bullets <-> platforms
export function bulletCollisionPlat() {
  for (let i = 0; i < bulletArray.bullets.length; i++) {
    for (let j = 0; j < platformArray.plat.length; j++) {
      if (
        bulletArray.bullets[i].x - platformArray.plat[j].x < 50 &&
        bulletArray.bullets[i].x - platformArray.plat[j].x > 0 &&
        bulletArray.bullets[i].y - platformArray.plat[j].y < 10 &&
        bulletArray.bullets[i].y - platformArray.plat[j].y > 0
      ) {
...
//Collision detection for bullets <-> bullets
export function bulletCollision() {
  for (let i = 0; i < bulletArray.bullets.length; i++) {
    for (let j = 0; j < bulletArray.bullets.length; j++) {
      if (
        bulletArray.bullets[i].x - bulletArray.bullets[j].x < 5 &&
        bulletArray.bullets[i].x - bulletArray.bullets[j].x > -5 &&
        bulletArray.bullets[i].y - bulletArray.bullets[j].y < 5 &&
        bulletArray.bullets[i].y - bulletArray.bullets[j].y > -5
      ) {
        if (i != j) {
...
//Collision detection for boss <-> bullets
export function bossHit() {
  for (let i = 0; i < bulletArray.bullets.length; i++) {
    for (let j = 0; j < 1; j++) {
      if (
        bulletArray.bullets[i].x - boss.x < 80 &&
        bulletArray.bullets[i].x - boss.x > 50 &&
        bulletArray.bullets[i].y - boss.y < 120 &&
        bulletArray.bullets[i].y - boss.y > 0
      ) {
